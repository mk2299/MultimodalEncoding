...
def audio_FPN():
    ...
    base_model = load_model('../Audio/vggish/vggish_weights_keras.h5')
    ...
    out = concatenate([layer4, GlobalAveragePooling2D()(p3), GlobalAveragePooling2D()(p4), GlobalAveragePooling2D()(p5)]) 
    model = Model(inputs = base_model.input, outputs = out)
    return model


def audio_FPN_frozen():
    ...
    print('VGG-ish layers not being trained')            
...
def visual_FPN(pretrained = True):
    ...
    if pretrained == True:
           print('ResNet layers not being trained')
           for layer in base_model.layers:
                   layer.trainable = False
    ...
    layer1 =  base_model.layers[38].output 
    layer2 =  base_model.layers[80].output 
    layer3 =  base_model.layers[142].output 
    layer4 =  base_model.get_output_at(-1)
