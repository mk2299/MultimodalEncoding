...
def main():
    ...
    from models_visual import pretrained_resnet_volumetric_context_FPN
    from dataloader_visual_volume import DataGenerator_vol_context
    ...
    train_generator = DataGenerator_vol_context(IDs_train, dim = (720,1024), batch_size = args.batch_size, train = True, delay = args.delay, time_steps = args.timesteps)
    val_generator = DataGenerator_vol_context(IDs_val, dim = (720,1024), batch_size = args.batch_size, train = False, delay = args.delay, time_steps = args.timesteps)
    ...
    model = pretrained_resnet_volumetric_context_FPN(img_shape = (args.timesteps, 720,1024,3), pretrained = bool(args.pretrained)) 
    if args.gpu_count>1:
